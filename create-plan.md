...
- 文件和内容搜索优先使用 `rg` / `rg --files`
- 计划阶段优先读取已有文档和路径索引，避免过早深入代码细节
- 用户确认按需进行，只在关键不确定点或高成本决策点打断

## 执行模式

### 标准模式（默认）
- 适用于中大型功能、方案权衡明显、需要正式计划评审的任务

### 轻量模式
- 适用于单模块改动、参考实现明确、交互复杂度较低的任务
- 保留 7 个必需章节，但每节只记录执行所需的核心信息
- 减少反复确认，优先复用现有分析、PRD 和设计文档
...
### 2. 检查需求分析报告、PRD 和设计文档（⚠️ 重要优化）

**优先级顺序**：设计文档 > 需求分析报告 > PRD > 直接需求

#### 2.1 搜索设计文档
```bash
glob ".ai-configs/design/**/*需求关键词*.md"
```

**如果找到设计文档**：
- ✅ 读取设计文档
- ✅ 提取界面结构、交互规则、状态设计、组件约束
- ✅ 将设计约束纳入实施步骤和验收方案
- ⚠️ 如果设计文档引用了 PRD 或分析报告，优先复用其路径
- ⏱️ **节省 20-40% 的交互澄清时间**
...
| 情况 | 策略 | 探索工作量 |
|------|------|-----------|
| **有设计文档 + 分析/PRD** | 设计约束 + 引用分析与 PRD | 0-20% |
| **有分析报告** | 直接复用探索结果 | 0-20% |
...
- 如果缺少设计文档但功能有明显交互复杂度："建议先运行 `/design` 补齐界面结构、交互规则和状态设计，可减少计划阶段反复澄清。"
...
2. **设计文档确认**
   - 若功能包含明显界面或交互变化且无法自动定位，再询问路径
   - 如果有，读取并理解设计方案中的交互和状态约束

3. **版本确认**（如果项目有多版本）
   - 仅在项目存在多版本分支或兼容差异时确认

4. **技术选型讨论**（如有多个方案）
   - 仅在方案差异会显著影响成本、风险或交互时确认

5. **不确定点确认**
   - API、交互或业务逻辑出现阻塞性歧义时再询问

### 分析过程

#### 深入探索
- 使用文件搜索和读取能力探索代码库，优先 `rg`
