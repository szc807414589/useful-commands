...
- `coding-implementer`：按计划落地实现、补充验证
- `repo-search`：变更点定位与影响面二次确认
- `code-reviewer`：交付前自检（轻量审查）

## 执行模式

### 标准模式（默认）
- 适用于多步骤开发任务、跨文件改动、需要完整进度记录的任务

### 轻量模式
- 适用于计划清晰、改动很小的任务
- 可合并相邻步骤执行，但仍需记录改动、验证和偏差
- 若执行中发现影响范围扩大，立即回到标准模式

## 输出契约（必须满足）

每次执行完成后，输出必须包含：

1. 计划执行进度（已完成/进行中/待执行）
2. 实际改动文件与原因
3. 关键实现说明（对应计划步骤）
4. 验证结果（构建/测试/手工验证）
5. 偏差说明（若与计划不一致，必须解释）
...
3. **读取相关设计文档**（如果计划或 PRD 中有引用）：
   - 优先读取 `.ai-configs/design/` 中对应功能的设计方案
   - 提取界面结构、组件规则、状态设计和交互约束
   - 如果没有设计文档，按编码计划执行，但对交互细节保持谨慎
