...
- 搜索内容优先使用 `rg`
- 搜索文件优先使用 `rg --files`
- 只有 `rg` 不可用时，才退回到其他等价工具
- 用户确认按需进行，不要求机械逐条提问

## 执行模式

### 标准模式（默认）
- 适用于新功能、复杂优化、影响范围不清晰的问题
- 完整做需求澄清、代码探索、影响面分析和可行性评估

### 轻量模式
- 适用于描述清晰、影响范围小、已有明显参考实现的问题
- 只补最关键的澄清和最小必要探索
- 输出仍保持 6 个部分，但内容可以更精简
...
# 搜索功能模块
rg --files src | rg "相关模块|Component名称|关键词"
```
...
**如果是交互复杂功能**（页面结构、弹窗、表单、状态较多）：
```bash
/design [需求描述]  # 生成设计方案（15-30分钟）
```
